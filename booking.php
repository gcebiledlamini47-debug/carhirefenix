...
// Enhanced with CSRF protection, input validation, prepared statements
// ============================================================
require_once 'db.php';
...
try {
    ...
    $vehicles = $db->query("SELECT id, name, plate, category FROM vehicles WHERE status = ? ORDER BY name", [VEHICLE_STATUS_AVAILABLE], 's');
    
    // Pre-selected vehicle
    $selected = null;
    if ($vehicle_id) {
        $selected = $db->queryOne("SELECT * FROM vehicles WHERE id = ? AND status = ?", [$vehicle_id, VEHICLE_STATUS_AVAILABLE], 'is');
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!SecurityHelper::verifyCSRFToken($csrf_token)) {
            ...
            $name       = Validator::sanitize($_POST['customer_name'] ?? '');
            $email      = Validator::sanitize($_POST['customer_email'] ?? '');
            $phone      = Validator::sanitize($_POST['customer_phone'] ?? '');
            $vid        = isset($_POST['vehicle_id']) ? (int)$_POST['vehicle_id'] : 0;
            $pickup     = Validator::sanitize($_POST['pickup_date'] ?? '');
            $returnDate = Validator::sanitize($_POST['return_date'] ?? '');
            $notes      = Validator::sanitize($_POST['notes'] ?? '');
            
            // Validate fields
            $validator->required('customer_name', $name);
            $validator->required('customer_phone', $phone);
            $validator->required('pickup_date', $pickup);
            $validator->integer('vehicle_id', $vid);
            
            if (!empty($email)) {
                $validator->email('customer_email', $email);
            }
            
            if (!empty($pickup)) {
                $validator->date('pickup_date', $pickup, 'Y-m-d');
                $validator->futureDate('pickup_date', $pickup, 'Y-m-d');
            }
            
            if (!empty($returnDate)) {
                $validator->date('return_date', $returnDate, 'Y-m-d');
            }
            ...
            if (!$validator->passes()) {
                ...
                // Get vehicle details
                $vehicle = $db->queryOne("SELECT id, name, plate FROM vehicles WHERE id = ? AND status = ?", [$vid, VEHICLE_STATUS_AVAILABLE], 'is');
                ...
                if (!$vehicle) {
                    ...
                    try {
                        // Save booking to database using prepared statement
                        ...
                        $db->update('vehicles', ['status' => VEHICLE_STATUS_RENTED], 'id = ?', [$vid], 'i');
                        
                        // ── SEND EMAIL TO ADMIN ──────────────────────────────
                        $to      = SUPPORT_EMAIL;
                        $subject = '[Fenix Car Hire] New Booking Request — ' . $ref;
                        
                        $vname = $vehicle['name'] . ' (' . $vehicle['plate'] . ')';
                        ...
                        if ($email) {
                            ...
                            $custHeaders  = "From: Fenix Car Hire <" . SUPPORT_EMAIL . ">\r\n";
                            $custHeaders .= "Reply-To: " . SUPPORT_EMAIL . "\r\n";
                        ...
                        // Log activity
                        SecurityHelper::logActivity('BOOKING_CREATED', "Booking $ref created by customer $name");
                        
                        $success = true;
                        
                    } catch (Exception $e) {
                        $error = 'Booking could not be saved. Please try again or call us on (+268) 76829797.';
                        SecurityHelper::logError('Booking save error', $e->getMessage());
                    }
                }
            }
        }
    ...
    $postName    = isset($_POST['customer_name'])  ? Validator::sanitize($_POST['customer_name'])  : '';
    $postEmail   = isset($_POST['customer_email']) ? Validator::sanitize($_POST['customer_email']) : '';
    $postPhone   = isset($_POST['customer_phone']) ? Validator::sanitize($_POST['customer_phone']) : '';
    $postPickup  = isset($_POST['pickup_date'])    ? Validator::sanitize($_POST['pickup_date'])    : '';
    $postReturn  = isset($_POST['return_date'])    ? Validator::sanitize($_POST['return_date'])    : '';
    $postNotes   = isset($_POST['notes'])          ? Validator::sanitize($_POST['notes'])          : '';
    $postVehicle = isset($_POST['vehicle_id'])     ? (int)$_POST['vehicle_id']                     : 0;

} catch (Exception $e) {
    $error = 'An error occurred. Please try again.';
    SecurityHelper::logError('Booking page error', $e->getMessage());
    $vehicles = [];
}
?>
        ...
        <div class="booking-success">
            ...
            <div class="success-details">
                <div class="success-row">
                    ...
                    <span class="success-val"><?php echo htmlspecialchars($ref); ?></span>
                </div>
                <div class="success-row">
                    <span class="success-label">Vehicle</span>
                    <span class="success-val"><?php echo htmlspecialchars($vehicle['name']) . ' (' . htmlspecialchars($vehicle['plate']) . ')'; ?></span>
                </div>
                <div class="success-row">
                    <span class="success-label">Pick-up Date</span>
                    <span class="success-val"><?php echo htmlspecialchars($pickup); ?></span>
                </div>
                <?php if ($returnDate): ?>
                <div class="success-row">
                    <span class="success-label">Return Date</span>
                    <span class="success-val"><?php echo htmlspecialchars($returnDate); ?></span>
                </div>
                <?php endif; ?>
            </div>
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <select name="vehicle_id" required>
                        ...
                        <option value="<?php echo $v['id']; ?>" <?php echo ($selected && $selected['id'] == $v['id']) || ($postVehicle == $v['id']) ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($v['name']); ?> — <?php echo htmlspecialchars($v['category']); ?> (<?php echo htmlspecialchars($v['plate']); ?>)
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
